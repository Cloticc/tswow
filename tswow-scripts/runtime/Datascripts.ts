...
export class Datascripts {
    ...
    static initialize() {
        if(!ipaths.node_modules.wowdata.exists()) {
            term.log('datascripts','Linking wowdata...');
            wsys.exec(`npm i -S ${ipaths.bin.scripts.wowdata.get()}`)
        }
    ...
    static async build(
        ...
        try {
            wsys.exec(
                ...
                + ` ${ipaths.bin.scripts.wowdata.wowdata.index.get()}`
                + ` --ipaths=./`
